  ...
  return (
    <div className="absolute inset-0 bg-black/80 backdrop-blur-sm rounded-2xl flex flex-col items-center justify-center z-10 p-4">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none rounded-2xl">
        <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-48 h-48 ${colors.bgGlow} rounded-full blur-3xl`}></div>
      </div>
          ...
          <div className="flex flex-wrap items-center justify-center gap-3 mt-2">
            {buttons.map((button, index) => {
              const btnGradient = button.color
                ...
                : colors.gradient;
              const btnShadow = button.color
                ? colorConfig[button.color]?.shadow || colors.shadow
                : colors.shadow;
